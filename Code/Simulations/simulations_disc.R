...
gen_data <- function(size,setting,seed){
  ...
  O1 <- matrix(rnorm(size*O1_dim,0,1),nrow=size,O1_dim)+10
  ...
  if(setting==1){
    ...
    Y1_mean <- function(O1,A1) .5*O1[,3]*A1 + cnst
    ...
    Y2_mean <- function(O1,Y1,A2) ((O1[,1]^2 + O1[,2]^2 - 0.2)*(0.5 - O1[,1]^2 - O1[,2]^1) + Y1)*A2 + cnst
...
psi4 <- function(x,y) (1+tanh(x))*(1+tanh(y))
...
#######################################################################
########### Simulations to check condition  #####################
#######################################################################


size <- 1000
df <- gen_disc_data(size,rnorm(1),ret='taus')

# Number of observations we are averaging across to check our
# condition C
disc_C <- table(Check_condition(df))
head(df[which(Check_condition(df)==1),])
disc_C 
...
disc_B <- table(Check_condition_B(df))
head(df[which(Check_condition_B(df)==0),])
...
feat_transf <- function(df,setting){
  ...
  if(setting==1 |setting==2){
    ...
    df.splines1 <- as.data.frame(lapply(nms1, function(nm) {ns(df.splines1[,nm],knots)}))
...
run_sims <- function(size,setting,sims_No){
  ...
  while (sim <= sims_No){
    ...
    start.time <- Sys.time()
    df.test <- fit.bowl(df.train,df.test,O1.vars,O2.bar.vars,surr_fn =  'exp')#'hinge')
    end.time <- Sys.time()
...
##    
    ...
    moCont <- buildModelObj(model = ~O1.1+O1.2+O1.2+A1+Y1,
                            solver.method = 'lm')
...
print(size)
size=15000
sim_res <- run_sims(size=as.numeric(size),setting='disc',sims_No=500)
...
saveRDS(sim_res,paste('res',size,'disc','.RDS',sep='_'))
